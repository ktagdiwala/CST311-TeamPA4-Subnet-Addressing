...
def genPrivateKey():
    ...
    subprocess.run(["sudo", "openssl", "genrsa", "-out", f"{chat_server_name}-key.pem", "2048"], cwd="/etc/ssl/demoCA")

# Function to generate the cert signing requests for the chat server
def genCSRs():
    print("Generating Certificate Signing Requests...")
    subprocess.run(["sudo", "openssl", "req", "-nodes", "-new", "-config", "/etc/ssl/openssl.cnf", "-key", f"{chat_server_name}-key.pem", "-out", f"{chat_server_name}.csr", "-subj", subject], cwd="/etc/ssl/demoCA")
